...
export interface WeatherData {
  ...
  windDirectionLabel: string; // N, NE, E, etc.
...
export interface WeatherForecast {
  current: WeatherData;
  hourly: (WeatherData & { time: string })[];
  daily: {
    date: string;
    tempHigh: number;
    tempLow: number;
...
const WEATHER_CODE_MAP: Record<number, { conditions: string; icon: string }> = {
...
function isGoodGolfWeather(temp: number, windSpeed: number, precipProb: number): boolean {
...
export function getWindImpact(windSpeed: number, windDirection: number, shotDirection: number): 'headwind' | 'tailwind' | 'crosswind' {
