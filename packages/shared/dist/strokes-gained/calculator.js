 ...
 */
import { getExpectedFromFairway, getExpectedFromRough, getExpectedFromBunker, getExpectedPutts, getExpectedOnGreen, getExpectedFromTee, } from "./benchmarks";
/**
 * Calculate Strokes Gained for a single hole
 */
export function calculateHoleStrokesGained(holeData, holeYardage = 400 // Default hole length if not provided
) {
 ...
 */
export function calculateRoundStrokesGained(holes, holeYardages) {
    const result = {
        sg_off_tee: 0,
        sg_approach: 0,
        sg_around_green: 0,
 ...
 */
export function calculateAverageStrokesGained(roundsData) {
    ...
    const totals = roundsData.reduce((acc, round) => ({
...
function estimateApproachDistance(par, holeYardage) {
...
function estimateFirstPuttDistance(gir, score, par, putts) {
...
function getDefaultYardage(par) {
    switch (par) {
        case 3:
            return 165;
        case 4:
...
function getExpectedFromMissedGreen(result, distance) {
...
function getExpectedFromChipPosition(result) {
 ...
 */
export function identifyWeakestArea(sg) {
    const areas = [
    ...
    const weakest = areas.reduce((min, current) => current.value < min.value ? current : min);
    return weakest;
}
/**
 * Identify the strongest area of the game based on Strokes Gained
 */
export function identifyStrongestArea(sg) {
    const areas = [
    ...
    return areas.reduce((max, current) => current.value > max.value ? current : max);
}
