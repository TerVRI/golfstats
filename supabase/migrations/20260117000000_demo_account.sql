...
-- 2. Run the seed-demo-account.sql script to populate data
--
-- Alternatively, run this migration after the user exists.
-- ============================================

-- Create a function to populate demo data (can be called after user creation)
CREATE OR REPLACE FUNCTION public.populate_demo_account()
RETURNS void AS $$
DECLARE
    demo_user_id UUID;
    demo_round_1 UUID;
...
BEGIN
    ...
    -- Exit if user doesn't exist yet
    IF demo_user_id IS NULL THEN
        RAISE NOTICE 'Demo user not found. Create user with email demo@roundcaddy.app first, then call this function.';
        RETURN;
    END IF;
    ...
    VALUES
        ...
        (demo_round_1, 4, 4, 3, 1, true, true, 0, 'Driver', '9 Iron', 'green', NULL, 8, 0.3, 0.5, 0.0, 0.4), -- Birdie!
        ...
        (demo_round_1, 18, 5, 5, 1, true, false, 0, 'Driver', '3 Wood', 'fringe', true, 6, 0.4, 0.1, 0.5, 0.2) -- Great up and down for par
    ...
    VALUES
        ...
        (demo_user_id, 'Eliminate 3-putts', 'No more than 1 three-putt per round', 'putting', 1, 2.5, CURRENT_DATE + INTERVAL '30 days', 'active'),
        (demo_user_id, 'Play 50 rounds this year', 'Get more time on the course', 'rounds', 50, 42, '2026-12-31', 'active'),
        (demo_user_id, 'Break 90', 'Consistently shoot under 90', 'score', 89, 85, CURRENT_DATE - INTERVAL '30 days', 'completed')
    ON CONFLICT DO NOTHING;
    ...
    RAISE NOTICE 'Demo account populated successfully!';
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Grant execute permission
GRANT EXECUTE ON FUNCTION public.populate_demo_account() TO service_role;
GRANT EXECUTE ON FUNCTION public.populate_demo_account() TO postgres;

-- ============================================
-- INSTRUCTIONS FOR APPLE APP REVIEW
-- ============================================
-- 
-- STEP 1: Create the demo user in Supabase Dashboard:
--   - Go to Authentication → Users → "Add user"
--   - Email: demo@roundcaddy.app
--   - Password: DemoReview2026!
--   - Check "Auto Confirm User"
--
-- STEP 2: Run this in the SQL Editor to populate demo data:
--   SELECT public.populate_demo_account();
--
-- Or run the seed-demo-account.sql script instead.
--
-- ============================================
-- DEMO CREDENTIALS FOR APPLE:
--   Email: demo@roundcaddy.app
--   Password: DemoReview2026!
-- ============================================
